...
if __name__ == '__main__':
    ...
    time_dblp = np.loadtxt(f'{input_path}/dblp_16_filtering.csv', delimiter=',')
    time_enron = np.loadtxt(f'{input_path}/enron_16_filtering.csv', delimiter=',')
    time_github = np.loadtxt(f'{input_path}/github_16_filtering.csv', delimiter=',')
    time_gowalla = np.loadtxt(f'{input_path}/gowalla_16_filtering.csv', delimiter=',')
    time_patents = np.loadtxt(f'{input_path}/patents_16_filtering.csv', delimiter=',')
    time_wikitalk = np.loadtxt(f'{input_path}/wikitalk_64_filtering.csv', delimiter=',')
    ...
    offsets = np.multiply(np.array(individual_speedup) - 1, np.array(ours))
    print(individual_speedup, np.average(individual_speedup), np.average(offsets))