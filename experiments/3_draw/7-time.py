...
if __name__ == '__main__':
    input_path = 'results/csv/scalability/nlabels'

    result_path = 'results/figs'
    os.makedirs(result_path, exist_ok=True)

    time_1 = np.loadtxt(f'{input_path}/enron_1_overall.csv', delimiter=',')
    time_4 = np.loadtxt(f'{input_path}/enron_4_overall.csv', delimiter=',')
    time_8 = np.loadtxt(f'{input_path}/enron_8_overall.csv', delimiter=',')
    time_16 = np.loadtxt(f'{input_path}/enron_16_overall.csv', delimiter=',')
    time_32 = np.loadtxt(f'{input_path}/enron_32_overall.csv', delimiter=',')
    ...
    fig.savefig(f'{result_path}/7-time.pdf')
    plt.close()
    print(cuts_time)
    print(ours_time)
