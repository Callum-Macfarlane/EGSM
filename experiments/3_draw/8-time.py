...
if __name__ == '__main__':
    input_path = 'results/csv/scalability/qsize'

    result_path = 'results/figs'
    os.makedirs(result_path, exist_ok=True)

    time_1 = np.loadtxt(f'{input_path}/enron_8_overall.csv', delimiter=',')
    time_4 = np.loadtxt(f'{input_path}/enron_10_overall.csv', delimiter=',')
    time_8 = np.loadtxt(f'{input_path}/enron_12_overall.csv', delimiter=',')
    time_16 = np.loadtxt(f'{input_path}/enron_14_overall.csv', delimiter=',')
    time_32 = np.loadtxt(f'{input_path}/enron_16_overall.csv', delimiter=',')
    ...
    axs.set_xlabel(f'$|V(Q)|$', fontsize=27)
    axs.set_xticks(np.arange(5))
    axs.set_xticklabels(['8', '10', '12', '14', '16'])
    ...
    fig.savefig(f'{result_path}/8-time.pdf')
    plt.close()
    print(cuts_time)
    print(ours_time)
