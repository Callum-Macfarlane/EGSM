        ...
        for i, algorithm in enumerate(algorithms):
            for j in range(100):
                ...
                if not os.path.exists(log_file):
                    ...
                    match = re.search('total time used: (.*)ms|,((?:\d|\.)*)ms,|\(host\) (\d*)\(kernel\)', full_txt)
                    #match = re.search(',((?:\d|\.)*)ms,|\(host\) (\d*)\(kernel\)', full_txt)
                    if match:
                        time_array[j, i] = float(match.group(i + 1))

        np.savetxt(f'{out_path}/{dataset}_{label}_overall_scatter.csv', time_array, fmt='%12.1f', delimiter=',')
